...
parse_standard_type :: proc(data : ^ParserData) -> StandardType {
    ...
    else if token == "__int64" { eat_token(data); return StandardType.Int64; }
...
parse_struct_type :: proc(data : ^ParserData, definitionPermitted : bool) -> IdentifierType {
    ...
    if !definitionPermitted || token != "{" {
      ...
      type.name = fmt.tprint(args={data.options.anonymousPrefix, "AnonymousStruct", anonymousStructCount}, sep="");
...
parse_union_type :: proc(data : ^ParserData) -> IdentifierType {
    ...
    if token != "{" {
      ...
      type.name = fmt.tprint(args={data.options.anonymousPrefix, "AnonymousUnion", anonymousUnionCount}, sep="");
...
parse_enum_type :: proc(data : ^ParserData) -> IdentifierType {
    ...
    if token != "{" {
      ...
      type.name = fmt.tprint(args={data.options.anonymousPrefix, "AnonymousEnum", anonymousEnumCount}, sep="");
    ...
    if token == "{" {
        ...
        node.name = type.name;
			...
			//fmt.println("enum def: ", node.name);
...
parse_enum_members :: proc(data : ^ParserData, members : ^[dynamic]EnumMember) {
    ...
    for token != "}" {
        ...
        if token == "," {
			...
			// @note Commented tool for debug
			//fmt.println("enum member: ", member.name);
  ...
  for token != "}" {
    ...
    if type, ok := member.type.base.(FunctionPointerType); ok {
      ...
      if !is_identifier(token) {
        member.name = fmt.tprint(args={"unamed", unamedCount}, sep="");
        unamedCount += 1;
      }
      else {
        member.name = parse_identifier(data);
