...
clean_identifier :: proc(name : string) -> string {
    ...
    if name[0] >= '0' && name[0] <= '9' {
      return fmt.tprint(args={"_", name}, sep="");
    }
...
clean_type :: proc(type : Type, options : ^GeneratorOptions, baseTab : string = "") -> string {
    ...
    for dimension in type.dimensions {
      output = fmt.tprint(args={output, "[", dimension, "]"}, sep="");
    }
    output = fmt.tprint(args={output, clean_base_type(type.base, options, baseTab)}, sep="");
...
clean_base_type :: proc(baseType : BaseType, options : ^GeneratorOptions, baseTab : string = "") -> string {
    if _type, ok := baseType.(BuiltinType); ok {
        ...
        return fmt.tprint(args={"^", name}, sep="");
    }
        ...
        output = fmt.tprint(args={output, parameters, ")"}, sep="");
...
clean_function_parameters :: proc(parameters : [dynamic]FunctionParameter, options : ^GeneratorOptions, baseTab : string) -> string {
    ...
    tab := "";
    if (len(parameters) > 1) {
      output = fmt.tprint(args={output, "\n"}, sep="");
      tab = fmt.tprint(args={baseTab, "    "}, sep="");
    }
    ...
    for parameter, i in parameters {
        ...
        if len(parameter.name) != 0 {
            ...
            name = fmt.tprint(args={"unamed", unamedParametersCount}, sep="");
            unamedParametersCount += 1;
        }

        output = fmt.tprint(args={output, tab, name, " : ", type}, sep="");
