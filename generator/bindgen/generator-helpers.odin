...
change_word_case :: proc(str : string, targetCase : WordCase) -> string {
    ...
    if targetCase == WordCase.Up {
        for c in str {
          newStr = fmt.tprint(args={newStr, to_uppercase(c)}, sep="");
        }
    }
    else if targetCase == WordCase.Low {
        for c in str {
          newStr = fmt.tprint(args={newStr, to_lowercase(c)}, sep="");
        }
    }
...
change_case :: proc(str : string, targetCase : Case) -> string {
    ...
    if targetCase == Case.Pascal {
        for part, i in parts {
          newStr = fmt.tprint(args={newStr, change_word_case(part, WordCase.FirstUpNumberReset)}, sep="");
        }
    }
    else if targetCase == Case.Snake {
        for part, i in parts {
          newStr = fmt.tprint(args={newStr, change_word_case(part, WordCase.Low), (i != len(parts) - 1) ? "_" : ""}, sep="");
        }
    }
    else if targetCase == Case.Kebab {
        for part, i in parts {
          newStr = fmt.tprint(args={newStr, change_word_case(part, WordCase.Low), (i != len(parts) - 1) ? "-" : ""}, sep="");
        }
    }
    else if targetCase == Case.Camel {
        for part, i in parts {
            if i == 0 {
              newStr = fmt.tprint(args={newStr, change_word_case(part, WordCase.Low)}, sep="");
            } else {
              newStr = fmt.tprint(args={newStr, change_word_case(part, WordCase.FirstUpNumberReset)}, sep="");
            }
        }
        ...
        for part, i in parts {
          newStr = fmt.tprint(args={newStr, change_word_case(part, WordCase.Up), (i != len(parts) - 1) ? "_" : ""}, sep="");
        }
    }
...
remove_prefixes :: proc(str : string, prefixes : []string, transparentPrefixes : []string = nil) -> string {
    ...
    for found {
        ...
        for prefix in transparentPrefixes {
            ...
            str[:len(prefix)] == prefix {
                str = str[len(prefix):];
                transparentStr = fmt.tprint(args={transparentStr, prefix}, sep="");
                if len(str) != 0 && (str[0] == '_' || str[0] == '-') {
                    str = str[1:];
                    transparentStr = fmt.tprint(args={transparentStr, '_'}, sep="");
                }
                found = true;
                break;
            }
    ...
    return fmt.tprint(args={transparentStr, str}, sep="");
}
...
remove_postfixes_with_removed :: proc(
    ...
    for found {
        ...
        for postfix in transparentPostfixes {
            ...
            str[len(str) - len(postfix):] == postfix {
                str = str[:len(str) - len(postfix)];
                transparentStr = fmt.tprint(args={postfix, transparentStr}, sep="");
                if len(str) != 0 && (str[len(str)-1] == '_' || str[len(str)-1] == '-') {
                    str = str[:len(str)-1];
                    transparentStr = fmt.tprint(args={'_', transparentStr}, sep="");
                }
                found = true;
                break;
            }
    ...
    return fmt.tprint(args={str, transparentStr}, sep=""), removedPostfixes;
}
